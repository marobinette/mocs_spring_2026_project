...
## This Project: The Diversity-Tension Kernel Extension

---

### The two processes happening simultaneously

The model has two things evolving at the same time: contagion spreading within groups, and people moving between groups. These feed back on each other.

---

### Process 1: Contagion within groups

Each person is either Susceptible (S) or Infected (I). Within a group of size n with i infected members, a susceptible person gets infected at rate:

**β(n, i) = λ · iᵛ**

| Parameter | What it is | Role |
|---|---|---|
| **λ** (lambda) | Baseline transmission rate | How infectious the disease/behavior is at the individual level. Swept on the x-axis of the heatmaps (log-spaced, 10⁻⁵ to 10⁻¹). |
| **ν** (nu) | Synergy exponent | Controls nonlinearity of reinforcement. At ν=1: classic linear contagion. At ν=10: having 5 infected neighbors is vastly more powerful than having 1. Swept on the y-axis (linear, 1 to 12). |
| **i** | Current infected count in a group | A live state variable — it changes moment to moment as contagion spreads and people recover. Not a parameter you set. |
| **μ** (mu) | Recovery rate | Set to 1. An infected person recovers at rate 1, becoming susceptible again. Sets the timescale for everything else. |

So λ and ν are parameters you set before running. i evolves dynamically during the simulation.

---

### Process 2: People moving between groups (switching/plasticity)

Independently of getting infected, people can leave their current group and join a random other group.

**In the paper (constant-ω model):**

ω = 5 — the same switching rate for everyone, regardless of which group they are in or how many infected people are in it. ω is a control parameter set from outside — it does not respond to the state of the system.

**In this project (diversity-tension kernel):**

**w(n, i) = α · (i/n) · (1 - i/n)**

| Component | What it is | Role |
|---|---|---|
| **i/n** | Infected fraction φ in this specific group right now | A live state variable. Two groups with different compositions have different switching rates. |
| **φ · (1 - φ)** | The diversity-tension shape | A parabola that peaks at φ=0.5 (maximally mixed group) and is zero at φ=0 (nobody infected) and φ=1 (everyone infected). Homogeneous groups have no tension. Maximally mixed groups have maximum tension. |
| **α** (alpha) | Kernel amplitude | The only free parameter introduced. Scales the whole switching rate up or down. Controls how strongly group composition drives switching behavior. α is a constant you set — but the switching rate itself varies dynamically with every group's state. |

At any moment, for any specific group, w depends on the live composition of that group. Two groups with different i/n have different switching rates even at the same α.

---

### The social mechanism

Laurent's framing: *"If the group is 50-50, people have to do too much code switching. Whereas if you're the only one in the minority, you just use the code of the other people. Or you find your people and stick with that."*

- **Homogeneous group** (φ=0 or 1): everyone agrees, no tension, nobody leaves → w = 0
- **Maximally mixed group** (φ=0.5): equal split, maximum tension, people flee → w = α/4
- **Partially mixed group**: somewhere in between, proportional tension

---

### How the two processes interact (the feedback loop)

1. Contagion spreads within groups → changes i → changes φ = i/n
2. φ changing → changes w(n,i) → changes how fast people leave that group
3. People leaving → reshuffles who is in which group → changes i values across multiple groups simultaneously
4. New i values → new infection pressures → contagion evolves differently → back to step 1

**In the paper, step 2 is broken.** ω is constant, so the state of contagion never affects switching. The arrow only goes one way: switching reshuffles groups, which affects contagion, but contagion never feeds back into switching.

**In this model the loop is closed.** That is the core conceptual contribution — the system is coevolutionary rather than driven.

---

### What the parameter sweep is actually doing

For each combination of (λ, ν) on the grid (50 × 40 = 2,000 cells), both processes run simultaneously to equilibrium — twice, from two different starting conditions:

- **I₀ = 0.001** (invasion attempt): starts near zero, climbs to whichever stable state it can reach from below
- **I₀ = 0.99** (persistence attempt): starts near full infection, relaxes to whichever stable state it can reach from above

The bistability indicator is:

**δI = I*(high) − I*(low)**

Large wherever two stable branches coexist. Zero wherever only one stable state exists.

The sweep over α asks the same question at different strengths of the feedback loop:

| α | Peak switching rate (α/4) | Bistable cells (δI > 0.05) |
|---|---|---|
| 0.5 | 0.125 | 150 — broad bistable region |
| 2.0 | 0.5 | 50 — region shrinking |
| 5.0 | 1.25 | 4 — nearly gone |
| 10.0 | 2.5 | 0 — bistability eliminated |
| 20.0–50.0 | 5.0–12.5 | 0 — fully suppressed |

**The critical α is between 5 and 10.** Above this threshold, the feedback loop is strong enough to erase the two-branch structure entirely — converting what is a discontinuous (first-order) phase transition in the constant-ω model into a continuous one.

Note: at α=20, the kernel's peak switching rate equals the paper's constant ω=5. Yet bistability is still suppressed. The suppression is not coming from a weaker switching rate — it is coming from the functional form. The paper's constant ω=5 keeps groups fluid even when they are homogeneous (φ=0 or 1). The kernel is zero at those endpoints, freezing the dynamics precisely where the two stable branches need to be sustained.

---

### The causal reversal (why this matters)

**Paper:** Group dynamics (plasticity rate ω) → shapes → contagion dynamics (phase portrait)

**This model:** Contagion dynamics (local prevalence i/n) → shapes → group dynamics (switching rate w) → feeds back into → contagion dynamics

In the paper, ω is a control parameter set from outside. In this model, the effective plasticity the system experiences is an emergent property — it depends on where the system is in the phase portrait at every moment. Early in an outbreak, groups are nearly homogeneous and w ≈ 0 (quasi-quenched). At endemic equilibrium, groups are again pushed to homogeneous endpoints and w → 0. The kernel is most active during the transient, when groups are mixed — and that is precisely when it disrupts the coexistence of branches that produces bistability.
