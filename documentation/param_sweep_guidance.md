...
## Initial results
  What Figure 3 shows (paper's result):

  - Panel a: the tricritical line in (ω, λ) space for the high school network. The dashed line at ω=5 cuts
  across the curve — the colored dots on it mark where each ν value crosses from continuous to discontinuous.
  Above the tricritical line = bistable/discontinuous. Below = continuous.
  - Panel b: I* vs λ for increasing ν, showing four distinct regimes (c–f): continuous → discontinuous with
  absorbing-active bistability → hybrid continuous with active bistability → three-state discontinuous hybrid.

  How our sweep maps onto this:

  The paper sweeps the (ω, λ) plane at fixed ν. We sweep the (λ, ν) plane at fixed ω. So our baseline heatmap
  is essentially a horizontal slice across Fig. 3a at ω=5, rotated 90°. Wherever our baseline δI > 0, that cell
   is in the "Discontinuous" region of Fig. 3a — which is exactly what the local sweep confirmed (bistability
  appearing around ν ≥ 5).

  The key new comparison:

  In the paper, the question is how much constant ω is needed to produce bistability. They find a non-monotonic
   invasion threshold — too little ω (quenched limit) gives no transition, moderate ω produces bistability,
  very high ω (annealed limit) pushes back toward continuous. Their ω=5 with the high school network sits
  squarely in the bistable regime.

  Your finding is: replace that constant ω=5 with a kernel of the same or even higher peak amplitude, and
  bistability collapses. At α=20 the kernel peaks at w=5 at φ=0.5 — identical to the paper's baseline at mixed
  groups — yet δI≈0. The suppression isn't coming from a weaker switching rate; it's coming from the functional
   form. The kernel is zero at the endpoints (φ=0, φ=1), which is exactly where the paper's model has ω=5
  keeping things active. That's what freezes the dynamics and kills the two-branch structure.

  The critical α between 5 and 10 means: once the kernel's peak amplitude (α/4) exceeds roughly 1.25–2.5, it's
  strong enough in mixed states to erase bistability even though it's still quenched at the endpoints. That is
  a genuinely new result relative to the paper — the paper only varies the level of ω, not its functional
  dependence on group state.
